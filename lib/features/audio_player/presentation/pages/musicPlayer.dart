...
import 'package:melody_player/features/home/presentation/pages/ListenNowPage.dart';
import 'package:melody_player/features/audio_player/presentation/bloc/key_player_cubit.dart';
import 'package:melody_player/features/audio_player/presentation/widgets/controller.dart';
...
class _MusicPlayerState extends State<MusicPlayer> {
  ...
  void didChangeDependencies() {
    ...
    setState(() {
      key = context.read<KeyPlayerCubit>().state;

      track = context.read<CurrentIndexCubit>().state;

      late List<listMusix.Track> tracks =
          context.read<AudioPlayerCubit>().state.trackList;
      a = List.from(tracks);
  ...
  Future<void> _playMusic(String text) async {
    ...
    try {
      ...
      List<AudioSource> child =
          context.read<AudioPlayerCubit>().state.playlist;
  ...
  Widget build(BuildContext context) {
    ...
    track = context.watch<CurrentIndexCubit>().state;
  ...
  void baralhar(BuildContext context) {
    if (_randomList == true) {
      a.shuffle();
      context.read<AudioPlayerCubit>().updateTrackList(a);
      context.read<CurrentIndexCubit>().updateCurrentIndexMusic(0);
    } else {
      context.read<AudioPlayerCubit>().updateTrackList(
          context.read<AudioPlayerCubit>().state.trackListEmOrdem);
      context.read<CurrentIndexCubit>().updateCurrentIndexMusic(0);
    }
